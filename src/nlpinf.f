C ======================================================================
C
C     NLPINF : CONSTRAINED NONLINEAR MAXIMUM-NORM DATA FITTING
C
C     NLPINF solves the constrained nonlinear maximum-norm data
C     fitting problem
C
C          min    max { |f_i(x)| : i = 1,...,L }
C          x in R^n :  g_j(x)  = 0 ,  j = 1,...,me
C                      g_j(x) >= 0 ,  j = me+1,...,m
C                      xl <= x <= xu
C
C     The objective function is non-differentiable, therefore one
C     additional variable and 2*L additional inequality
C     constraints are introduced to get an equivalent smooth
C     nonlinear program.  The code is particularly useful for
C     nonlinear approximation problems with a large number of
C     support values.
C
...
C                least N + L + 1.
C      LMNN2   : Dimension of U, must be equal to M + 2*L + 2*N + 4.
...
C ======================================================================
C
      SUBROUTINE NLPINF (L, M, ME, LMMAX, N, LNMAX, LMNN2, X, F, G,
...
C
     ...
     /   THEN
      ...
      IF (LWW.LT.1) THEN
         ...
         RETURN
      ENDIF
      CALL NLDFTW (3, L, M, ME, LMMAX, N, LNMAX, LMNN2, X, F, G,
