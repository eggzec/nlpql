C ======================================================================
C
C     NLPLSX : LEAST SQUARES WITH A VERY LARGE NUMBER OF TERMS
C
C     NLPLSX solves the constrained nonlinear least squares
C     problem
C
...
C                      xl <= x <= xu
C
C     where the number L of squared terms is very large.  No
C     additional variables are introduced in this case, since
C     their number would be prohibitive.  The sum of squares and
C     its gradient are assembled from the individual terms.
C
...
C                least N + L + 1.
C      LMNN2   : Dimension of U, must be equal to M + 2*N + 2.
...
C ======================================================================
C
      SUBROUTINE NLPLSX (L, M, ME, LMMAX, N, LNMAX, LMNN2, X, F, G,
...
C
     ...
     /   THEN
      ...
      IF (LWW.LT.1) THEN
         ...
         RETURN
      ENDIF
      CALL NLDFTW (5, L, M, ME, LMMAX, N, LNMAX, LMNN2, X, F, G,
