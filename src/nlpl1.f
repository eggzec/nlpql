C ======================================================================
C
C     NLPL1 : CONSTRAINED NONLINEAR L1 DATA FITTING
C
C     NLPL1 solves the constrained nonlinear L1 problem
C
C          min    sum |f_i(x)| ,  i = 1,...,L
C          x in R^n :  g_j(x)  = 0 ,  j = 1,...,me
C                      g_j(x) >= 0 ,  j = me+1,...,m
C                      xl <= x <= xu
C
C     The objective function is non-differentiable, therefore L
C     additional variables and 2*L additional inequality
...
C                least N + L + 1.
C      LMNN2   : Dimension of U, must be equal to M + 4*L + 2*N + 2.
...
C ======================================================================
C
      SUBROUTINE NLPL1 (L, M, ME, LMMAX, N, LNMAX, LMNN2, X, F, G,
...
C
     ...
     /   THEN
      ...
      IF (LWW.LT.1) THEN
         ...
         RETURN
      ENDIF
      CALL NLDFTW (2, L, M, ME, LMMAX, N, LNMAX, LMNN2, X, F, G,
